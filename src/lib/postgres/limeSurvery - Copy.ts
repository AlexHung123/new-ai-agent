    ...
    const columns: any[] = await prisma.$queryRawUnsafe(
      `WITH qs AS (
        ...
        GROUP BY
            ...
            qs.sid, qs.gid, qs.question_order
        ),
        flat_cols AS (
        SELECT
            qid, qtext, question_theme_name, type, question_order,
        ...
        GROUP BY t.question_order, t.qtext
        ),

        merged AS (
        SELECT question_order, qtext, jarr FROM arr_json
        UNION ALL
        SELECT question_order, qtext, jarr FROM text_answers
        )
        SELECT
        jsonb_agg(
